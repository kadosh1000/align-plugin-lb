...
class WaiterHelper {
     ...
     */
    async waitForFreeAgents(agentTags, projectId, { checkInterval, maxRetries, currentRetry }) {
    ...
    async waitForAllAgents(agentTags, { checkInterval, maxRetries, currentRetry }) {
        const agents = await dbHelper.getLivingAgents(agentTags);
        ...
        // Continue for the next retry
            ...
            const nextRetry = currentRetry ? currentRetry + 1 : 2;
            // console.log(`Failed to find, continue to try ${nextRetry}`);
            return this.waitForAllAgents(agentTags, {
                checkInterval,
                maxRetries,
                currentRetry: nextRetry,
            });
